...
class IntermediateProcessor():
    ...
    def find_weld_boundaries(self, path_weld_mask, mask_name):
        """ Определяет границы шва по маске. """
        img = cv2.imread(path_weld_mask + mask_name, -1)
        paint = cv2.imread(path_weld_mask + mask_name)
        ...
        if y1 == 0 and y2 == 0 and i == 0:
            ...
            cv2.imwrite(path_weld_mask + mask_name, paint)
            return 0
    ...
    def get_weld_and_gray_bgr(self, path_processing_img, process_img_name):
        """ Заполняет фон (т.е. то, что не является областью шва) серым цветом. """
        processing_img = cv2.imread(path_processing_img + process_img_name, 0)
    ...
    def form_weld_area(self, path_scale_weld, process_img_name):
        ...
        cv2.imwrite(path_scale_weld + process_img_name, scale_img)
